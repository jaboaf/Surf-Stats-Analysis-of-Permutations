...
struct FiniteFn{DomainT,RangeT} 
	D::Array{DomainT}
	R::Array{RangeT}
	FiniteFn(D::Array{DomainT},R::Array{RangeT}) where {DomainT,RangeT} = allunique(D) ? new{DomainT,RangeT}(D,R) : new{DomainT,RangeT}(unique(D),[union(R[findall(==(x),D)]) for x in unique(D)])
end
