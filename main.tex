...
\begin{definition} A linear or "faithful" representation of a group G, is the ordered pair $(\rho, V)$, $\rho: G \rightarrow GL(V)$ satisfying:
...
\( \forall x \in G, \rho(x^{-1}) = \rho(x)^*, \text{ where * denotes involution in GL(V)} \).
\end{definition}
Note: This amounts to saying that $\rho$ is a group hom from $(G, \circ)$ to $GL(V)$

\begin{definition}The representation of a measure F, is: $ \hat{F} := \sum_{\tau \in S_n} P(\tau) \rho(\tau) $. \end{definition}
Note: This is sometimes called the Fourier transform at a representation, I avoid that 

Let $ H := S_n / ^{-1}$ 
$ h in H \iff \forall x,y \in h x*y = |
...
\begin{definition}(Convolution of two functions on a group G) is a binary operation $ (A * B)(g) = \sum_{\tau \in S_n} a(\tau) b(\tau^{-1}g) $. equivalently:
\end{definition}

Note: $ \widehat{A*B} = \sum_{\gamma} (A*B)(\gamma)\rho(\gamma) 
= \sum_{\gamma } \sum_{\tau} a(\gamma \tau^{-1})b(\tau)\rho(\gamma ) $

We take $\rho_M $ the permutation representation acting on the vector space $V := \mathbb{R}^n$ with basis indexed by OneTon $ := \{1, \dots n\} $. So a typical element of V is of the form: $ \begin{bmatrix} a_1 \\ a_2 \\ \vdots \\ a_n \end{bmatrix}$ which could be rewritten as $\sum_{i \in \text{OneTon} } a_i e_i$. But remember, we are interested in functions that act on the basis. Our interest is specifically permutations of elements of OneTon, denoted $S_n $
